...
class StackVisualizer2D:
    def __init__(self, root):
        ...
        is_full_button = tk.Button(self.button_frame, text="Is Full?", command=self.is_full, **button_style)
        is_full_button.grid(row=0, column=6, padx=1, pady=1)
    ...
    def update_visual(self):
        ...
        if not elements:
            ...
            self.canvas.create_text(self.canvas_width / 2, self.container_base_y + 20,
                                    text="Top = -1", font=("Arial", 14), fill="black")
    ...
    def draw_stack_elements(self, elements):
        ...
        for i, value in enumerate(reversed(elements)):
            y_position = self.container_base_y - (i + 1) * box_height - box_height / 2  # no gap

            self.canvas.create_rectangle(start_x, y_position, start_x + box_width, y_position + box_height,
                                         fill="lightblue", outline="black")
            self.canvas.create_text(start_x + box_width / 2, y_position + box_height / 2,
                                    text=str(value), font=("Arial", 12))
...
if __name__ == "__main__":
    ...
    title_label = tk.Label(root, text="Implementation of Stacks using Arrays", font=("Arial", 16), pady=3)
    title_label.pack()

    max_size_label = tk.Label(root, text="Maximum size of Array is 10", font=("Arial", 12), fg="blue")
    max_size_label.pack(pady=1)
